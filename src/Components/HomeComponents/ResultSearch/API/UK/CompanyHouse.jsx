...
const CompanyHouse = ({ companyNumber }) => {
  ...
  const SetDataBol = (bol1, bol2, bol3, bol4) => {
    setCharge(bol1);
    setPeople(bol2);
    setCompany(bol3);
    setHistory(bol4);
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex gap-[2px] mt-10  print:!mt-0 w-full ">
        <button
          // href={`/uk/${}`}
          onClick={() => {
            SetDataBol(false, false, true, false);
            setId(results?.company_number);
          }}
          className={`${
            Company ? "bg-slate-600" : "bg-slate-400"
          } p-1 text-white border hover:opacity-50 border-black transition-all duration-100  text-lg  font-bold`}
        >
          Company house
        </button>
        <button
          onClick={() => {
            SetDataBol(true, false, false, false);
            setId(results?.company_number);
          }}
          className={`${
            Charge ? "bg-slate-600" : "bg-slate-400"
          }   p-1 text-white border hover:opacity-50 border-black transition-all duration-100 text-lg font-bold`}
        >
          Charges
        </button>
        <button
          onClick={() => {
            SetDataBol(false, true, false, false);
            setId(results?.company_number);
          }}
          className={`${
            People ? "bg-slate-600" : "bg-slate-400"
          } p-1 text-white border hover:opacity-50 border-black transition-all duration-100 text-lg font-bold`}
        >
          Peeple
        </button>
        <button
          onClick={() => {
            SetDataBol(false, false, false, true);
            setId(results?.company_number);
          }}
          className={`${
            history ? "bg-slate-600" : "bg-slate-400"
          } p-1 text-white border hover:opacity-50 border-black transition-all duration-100 text-lg font-bold`}
        >
          History
        </button>
      ...
      <div className="flex flex-col gap-4">
        {Company && (
          <div className="border w-full m-auto border-gray-300 rounded-lg p-8 shadow bg-white">
                ...
                <div className="col-span-6 mt-4 text-xl">
                  ...
                  <div className="text-lg">
                    ...
                    <p className="font-normal">
                      ...
                      <span className="font-bold">
                        {results.accounts?.last_accounts.made_up_to}
                      </span>
                    </p>
                  </div>
                </div>
        ...
        {People && officersResult.items?.length ? (
                ...
                <div className="grid grid-cols-12 ">
                  <div className="flex items-center gap-2 col-span-12 font-bold mb-2 text-lg">
                    ...
                    <Link
                      ...
                      target="_blank"
                      className="flex items-center gap-3 hover:underline transition-all duration-100"
                    >
                      {officer.name}{" "}
                      <FontAwesomeIcon icon={faUpRightFromSquare} className="w-5 h-5  mb-1" />
                    </Link>
                  </div>
      ...
      {Charge && <Charges id={companyNumber} />}
      {history && <FilingHistory companyNumber={companyNumber} />}
    </div>
  );
};
